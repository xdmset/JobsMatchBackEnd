...
def _generate_with_claude(context: dict[str, Any]) -> dict[str, Any]:
    """
    Genera el roadmap usando el SDK oficial de Anthropic (Claude).
    """
    if not settings.ROADMAP_AI_API_KEY:
        raise ValueError("ROADMAP_AI_API_KEY no está configurada")

    logger.info(
        "Llamando a Claude SDK | modelo=%s | timeout=%ss",
        settings.ROADMAP_AI_MODEL,
        settings.ROADMAP_AI_TIMEOUT_SECONDS,
    )

    client = anthropic.Anthropic(
        api_key=settings.ROADMAP_AI_API_KEY,
    ...
    with client.messages.stream(
        ...
        messages=[
            {
                "role": "user",
                "content": "Genera el roadmap personalizado en JSON según las instrucciones del sistema.",
            }
        ],
    ) as stream:
    ...
    logger.info(
        "Respuesta de Claude SDK recibida | stop_reason=%s | output_tokens=%s",
    ...
    if content is None:
        raise ValueError("Claude no devolvió contenido de texto en la respuesta")
...
def _generate_roadmap_payload(context: dict[str, Any]) -> dict[str, Any]:
    ...
    logger.info("Intentando generación con Claude SDK (ROADMAP_AI_MODE=%s)", settings.ROADMAP_AI_MODEL)
    return _generate_with_claude(context)
